...
# Get the clean file content directly from git
...
# Update Exp 2 (Puhu Electronics) EN & TR 
# (Since these were also updated in the first session yesterday)
new_exp2_b1 = r"exp2_b1: 'Developed flight computer architecture and telemetry modules.',"
new_exp2_b2 = r"exp2_b2: 'Integrated and tested autonomous control algorithms.',"
js = re.sub(r"exp2_b1:.*?,", new_exp2_b1, js, count=1, flags=re.DOTALL)
js = re.sub(r"exp2_b2:.*?,", new_exp2_b2, js, count=1, flags=re.DOTALL)

new_exp2_tr_b1 = r"exp2_b1: 'Uçuş bilgisayarı mimarisi ve telemetri modülleri geliştirildi.',"
new_exp2_tr_b2 = r"exp2_b2: 'Otonom kontrol algoritmaları entegre edildi ve test süreçleri yürütüldü.',"
js = re.sub(r"exp2_b1:.*?,", new_exp2_tr_b1, js, count=1, flags=re.DOTALL)
js = re.sub(r"exp2_b2:.*?,", new_exp2_tr_b2, js, count=1, flags=re.DOTALL)
